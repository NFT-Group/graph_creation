...
def mergeCSV(CSV1, CSV2, MergedCSV, DollarCSV):
    data1 = pd.read_csv(CSV1)
    data2 = pd.read_csv(CSV2)

    output1 = pd.merge(data1, data2, on='date', how='inner')
    
    output1.to_csv(MergedCSV)

def addDollarPrice(CSV, DollarCSV):
    mergeCSV(CSV, DollarCSV, 'temp.csv', 1)
    temp = pd.pandas('temp.csv')
    output1 = pd.read_csv(CSV)
    output1['dollar_price'] = temp['Close']
    output1.to_csv(MergedCSV)

def attachDollarPrice(MergedCSV, DollarCSV):
    dollar = pd.read_csv(DollarCSV)
    output1 = pd.read_csv(MergedCSV)
    output1.columns = ['date', 'bored_apes_1', 'cool_cats_1']
    output1['dollar_price'] = dollar['Close']
...
findFirstLastDates("CoolCats2.csv", "BoredApes2.csv")
EthToUSD = 2652
mergeCSV("CoolCats2.csv", "BoredApes2.csv", "MergedCSV.csv", "ETH-USD.csv")
